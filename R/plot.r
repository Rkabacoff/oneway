...
#' @param plot a character string representing the type of plot. Default is \code{boxplot}.
#' Other types of plot: \code{violin} and \code{density_ridges}.
...
plot.oneway <- function(x, plot = "boxplot", ...){
  ...
  # density ridges plot
    ...
    ggplot(x$anova$model, aes(.data[[y]], .data[[g]],group=.data[[g]],fill=.data[[g]])) +
      ...
      ggridges::theme_ridges() + labs(x=g) +
      ggtitle("Density ridges plot")
  }
}
